(() => {
  ...
  // extensions/recycle-fee/src/Checkout.jsx
...
${errorInfo.componentStack}`);
  ...
  function App() {
    ...
    const [product, setProduct] = (0, import_react11.useState)();
    const [showFeeMessage, setShowFeeMessage] = (0, import_react11.useState)(false);
    const lines = useCartLines([]);
    const feeLocations = ["CT", "RI", "CA"];
    ...
    (0, import_react11.useEffect)(() => {
      ...
      if (feeLocations.includes(shippingAddress.provinceCode)) {
        for (const variant of product.variants.edges) {
          if (variant.node.selectedOptions[0].value === shippingAddress.provinceCode) {
        ...
        if (feeId !== "") {
          for (const line of lines) {
            if (line.merchandise.product.productType === "mattress" || line.merchandise.product.productType === "Box Spring") {
              console.log("mattress or box spring found");
              setShowFeeMessage(true);
              feeNeeded = feeNeeded + line.quantity;
            } else if (line.merchandise.product.id === (product == null ? void 0 : product.id)) {
              foundLineIds.push(line.id);
              foundLineQtys.push(line.quantity);
              feeFound = feeFound + line.quantity;
            }
          }
          console.log("feeNeeded: ", feeNeeded);
          console.log("feeFound: ", feeFound);
          console.log("foundLineIds: ", foundLineIds);
          console.log("foundLineQtys: ", foundLineQtys);
          ...
          if (!feeLocations.includes(shippingAddress.provinceCode)) {
            for (const lineId of foundLineIds) {
              ...
              handleRemoveFromCart(lineId, foundLineQtys[foundLineIds.indexOf(lineId)]);
            }
          }
          if (feeFound > feeNeeded) {
            for (const lineId of foundLineIds) {
              setShowFeeMessage(false);
              handleRemoveFromCart(lineId, foundLineQtys[foundLineIds.indexOf(lineId)]);
            }
    ...
    (0, import_react11.useEffect)(() => {
      ...
      console.log("product: ", product);
      if (product && (product == null ? void 0 : product.variants.length) > 0) {
        console.log("product.variants found");
        checkForRecycleFee();
      } else {
        console.log("useEffect shipping address fetching product");
        fetchProduct().then((product2) => {
    ...
    function checkForRecycleFee() {
      if (!product)
        return;
      const provinceCode = shippingAddress == null ? void 0 : shippingAddress.provinceCode;
      let feeId = "";
      if (feeLocations.includes(provinceCode)) {
        ...
        console.log("province code does not require a recycle fee");
        for (const line of lines) {
          console.log("line.merchandise.product: ", line.merchandise.product);
          console.log("product?.id: ", product == null ? void 0 : product.id);
    ...
    function handleAddToCart(variantId, quantity) {
      return __async(this, null, function* () {
    ...
    function handleRemoveFromCart(lineId, quantity) {
      return __async(this, null, function* () {
        yield applyCartLinesChange({
          type: "removeCartLine",
          id: lineId,
    ...
    ;
    if (showFeeMessage) {
      return /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Banner2, { title: "Recycling Fee Required", status: "warning", children: "This purchase requires a recycling fee, which we have added to your cart." });
    } else {
      return null;
    }
  }
})();
//# sourceMappingURL=recycle-fee.js.map
